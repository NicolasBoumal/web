...
% function M = stiefelstackedfactory(m, d, k)
%
% This factory describes a Riemannian manifold for use with Manopt, the
% toolbox for optimization on manifolds. Points on this manifold are
% matrices Y of size n x k, with n = m*d. Y is thought of as m matrices of
% size d x k each stacked on top of each other. Call them Y1, ..., Ym. Each
% Yi is an orthonormal matrix, that is, its d rows are unit norm and are
% orthogonal to each other. Thus, this geometry is a product of Stiefel
% manifolds. To easily transform matrices Y to 3D arrays Y3 of size
% d x k x m such that each slice Y3(:, :, i) corresponds to one of the
% matrices Yi, use the functions
% 
...
% useful.
%
% See also: stiefelfactory multiprod multitransp
%
% Nicolas Boumal, UCLouvain, May 13, 2014.
    ...
    % Given 2 matrices A and B of size dxkxm, returns a matrix C of size
    % dxdxm such that each slice C(:, :, i) is the symmetric part of the
    % product A(:, :, i) * B(:, :, i)'.
    M.symbdiag = @symbdiag;
